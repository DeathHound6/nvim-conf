...
vim.keymap.set("n", "gwvd",
    function()
        vim.cmd([[vsplit]])
        vim.lsp.buf.definition()
    end,
    { desc = "`gwvd` jumps to the highlighted symbol's definition in a new buffer (vsplit)" }
)
vim.keymap.set({'n', 'v'}, '<C-n>', ":set hlsearch!", { desc = "'Ctrl + n' toggles search highlighting" })
